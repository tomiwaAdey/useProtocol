...
export interface Database {
	addr(name: string, coinType: number, env: Env): PromiseOrResult<{ addr: string; ttl: number }>;
	text(name: string, key: string, env: Env): PromiseOrResult<{ value: string; ttl: number }>;
	contenthash(name: string, env: Env): PromiseOrResult<{ contenthash: string; ttl: number }>;
}
